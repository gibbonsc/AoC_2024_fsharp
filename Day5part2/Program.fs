...
let main(args: string array) : int =
    ...
    *)
    ...
    let (invalidProdUpdateLists: uint list list) =
        prodUpdateLists
        |> List.filter (fun x -> (allProdUpdatesFollowOrderRules x) <> true)

    // let sidePrintf (nums: uint list) =
    //     nums
    //     |> List.iter (printf " %u")
    //     printfn ""
    // invalidProdUpdateLists
    // |> List.iter sidePrintf

    let compareByOrderRule (a: uint) (b: uint) : int =
        if a = b then 0
        elif (followsAnOrderRule (a,b)) then -1
        else 1
    let sortInvalidProdUpdateList (el: uint list) : uint list =
        List.sortWith compareByOrderRule el
    let sortedInvalidProdUpdateLists: uint list list =
        invalidProdUpdateLists
        |> List.map sortInvalidProdUpdateList

    // let sidePrintf (nums: uint list) =
    //     nums
    //     |> List.iter (printf " %u")
    //     printfn ""
    // sortedInvalidProdUpdateLists
    // |> List.iter sidePrintf
